...
type PushBatchReport = {
  batch: 'K2.8c-FreeOpsRuntimeSubset';
  runDate: string;
  baseUrl: string;
  resolveIp?: string;
  headBefore: string;
...
const K28C_TASK = {
  taskId: 'K28C-T01' as const,
  title: 'free class_1 journey date whitespace validation push',
  instruction:
    'In `server/src/routes/journey.ts`, tighten only the existing `/optimal-dates` route so whitespace-only `startDate`, `endDate`, or `birthDate` are rejected before calculation. Add local trimmed `startDate`, `endDate`, and `birthDate` variables inside the route handler, keep the existing `400` payload `{error:\'invalid_request\', message:\'eventType, startDate, endDate, birthDate required\'}` when any required field is empty after trimming, and pass the trimmed dates into `calculate(...)` without changing any other route, helper, type, or file. Do not change `eventType` semantics and do not touch any other file.',
  ...
  expectedStrings: {
    ...
    oldValidation: "    if(!body.eventType||!body.startDate||!body.endDate||!body.birthDate) {",
    oldCalculate: '    const result = calculate(body);',
    newTrimmedStart: "    const startDate = (body.startDate ?? '').trim();",
    newTrimmedEnd: "    const endDate = (body.endDate ?? '').trim();",
    newTrimmedBirthDate: "    const birthDate = (body.birthDate ?? '').trim();",
    newCalculate: '    const result = calculate({...body, startDate, endDate, birthDate});',
  },
};
...
function getDefaultOutputPath(repoRoot: string): string {
  return path.resolve(repoRoot, '..', 'k28c-free-runtime-results.json');
}
...
async function main() {
  ...
  if (!token) throw new Error('OPUS_BRIDGE_SECRET missing for K2.8c runner');

  const baseUrl = (process.env.K28C_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? 'https://soulmatch-1.onrender.com').replace(/\/$/, '');
  const resolveIp = process.env.K28C_RESOLVE_IP ?? process.env.K28B_RESOLVE_IP ?? process.env.K28A_RESOLVE_IP ?? process.env.K27F_RESOLVE_IP ?? process.env.K27E_RESOLVE_IP ?? process.env.K27D_RESOLVE_IP ?? process.env.K27C_RESOLVE_IP ?? process.env.DEPLOY_RESOLVE_IP;
  const dispatcher = createResolveDispatcher(baseUrl, resolveIp);
  const outputFile = getDefaultOutputPath(repoRoot);

  const currentContent = normalizeText(await readRepoFile(repoRoot, K28C_TASK.expectedChangedFiles[0]));
  if (
    currentContent.includes(K28C_TASK.expectedStrings.newTrimmedStart)
    || currentContent.includes(K28C_TASK.expectedStrings.newTrimmedEnd)
    || currentContent.includes(K28C_TASK.expectedStrings.newTrimmedBirthDate)
  ) {
    throw new Error(`K28C-T01 already landed on the current repo truth (${K28C_TASK.expectedChangedFiles[0]} already contains trimmed journey date validation).`);
  }
  if (
    !currentContent.includes(K28C_TASK.expectedStrings.routeStart)
    || !currentContent.includes(K28C_TASK.expectedStrings.oldValidation)
    || !currentContent.includes(K28C_TASK.expectedStrings.oldCalculate)
  ) {
    throw new Error(`K28C-T01 cannot run because the expected journey route anchors are missing from ${K28C_TASK.expectedChangedFiles[0]}.`);
  }

  const routeUrl = `${baseUrl}/api/journey/optimal-dates`;
  const preflightInvalidProbe = await fetchProbe(routeUrl, K28C_TASK.invalidBody, dispatcher);
  const preflightValidProbe = await fetchProbe(routeUrl, K28C_TASK.validBody, dispatcher);
  if (preflightInvalidProbe.status !== 200) {
    throw new Error(`K28C-T01 expected whitespace-only startDate preflight probe to remain open on current runtime, got ${preflightInvalidProbe.status}.`);
  }
  if (preflightValidProbe.status !== 200) {
    throw new Error(`K28C-T01 expected valid preflight probe to succeed, got ${preflightValidProbe.status}.`);
  }
  ...
  const postDeployInvalidProbe = await fetchProbe(routeUrl, K28C_TASK.invalidBody, dispatcher);
  const postDeployValidProbe = await fetchProbe(routeUrl, K28C_TASK.validBody, dispatcher);
  ...
  if (taskClass !== K28C_TASK.expectedTaskClass) assessmentReasons.push(`expected taskClass ${K28C_TASK.expectedTaskClass}, got ${taskClass}`);
  ...
  if (JSON.stringify(changedFiles) !== JSON.stringify(K28C_TASK.expectedChangedFiles)) {
    assessmentReasons.push(`expected changedFiles ${K28C_TASK.expectedChangedFiles.join(', ')}, got ${changedFiles.join(', ') || 'none'}`);
  }
  ...
  if (!invalidWhitespaceRejected) assessmentReasons.push(`expected whitespace-only startDate probe to return 400 invalid_request, got ${postDeployInvalidProbe.status}`);
  ...
  report.task = {
    taskId: K28C_TASK.taskId,
    title: K28C_TASK.title,
    status,
    ...
    verifiedCommit,
    changedFiles,
    scopeClean: changedFiles.every((filePath) => K28C_TASK.expectedChangedFiles.includes(filePath)),
