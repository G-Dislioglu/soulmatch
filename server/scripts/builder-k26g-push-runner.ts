...
type PushBatchReport = {
  batch: 'K2.6g-CodeAdjacentControlledPush';
  runDate: string;
  baseUrl: string;
  resolveIp?: string;
  headBefore: string;
...
const K26G_TASK = {
  taskId: 'K26G-T01' as const,
  title: 'controlled class_1 numerology whitespace validation push',
  instruction:
    'In `server/src/routes/numerology.ts`, tighten only the existing `/calc` request validation so whitespace-only `profileId`, `name`, and `birthDate` no longer pass. Add local trimmed string normalization for those three fields inside the route handler, use the trimmed values in the required-field and birth-date checks, and pass the normalized values into `calculateNumerology(...)` plus the success logger. Keep the same route shape and do not modify any other file.',
  scope: ['server/src/routes/numerology.ts'],
  expectedTaskClass: 'class_1' as const,
  expectedChangedFiles: ['server/src/routes/numerology.ts'],
  invalidBody: '{"profileId":"   ","name":"   ","birthDate":"2020-01-02","system":"pythagorean"}',
  validBody: '{"profileId":"abc","name":"Ada","birthDate":"2020-01-02","system":"pythagorean"}',
  expectedStrings: {
    oldRequired: "if (!request.profileId || !request.name || !request.birthDate || !request.system) {",
    oldBirthDate: "if (!/^\\d{4}-\\d{2}-\\d{2}$/.test(request.birthDate)) {",
    oldResult: 'const result = calculateNumerology(request);',
    newProfileId: "const profileId = typeof request.profileId === 'string' ? request.profileId.trim() : '';",
    newName: "const name = typeof request.name === 'string' ? request.name.trim() : '';",
    newBirthDate: "const birthDate = typeof request.birthDate === 'string' ? request.birthDate.trim() : '';",
  },
};
...
function getDefaultOutputPath(repoRoot: string): string {
  return path.resolve(repoRoot, '..', 'k26g-push-results.json');
}
...
async function main() {
  ...
  if (!token) {
    throw new Error('OPUS_BRIDGE_SECRET missing for K2.6g runner');
  }

  const baseUrl = (process.env.K26G_BASE_URL ?? process.env.K26F_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? 'https://soulmatch-1.onrender.com').replace(/\/$/, '');
  const resolveIp = process.env.K26G_RESOLVE_IP ?? process.env.K26F_RESOLVE_IP ?? process.env.DEPLOY_RESOLVE_IP;
  ...
  const currentContent = normalizeText(await readRepoFile(repoRoot, K26G_TASK.expectedChangedFiles[0]));
  if (currentContent.includes(K26G_TASK.expectedStrings.newProfileId) || currentContent.includes(K26G_TASK.expectedStrings.newName) || currentContent.includes(K26G_TASK.expectedStrings.newBirthDate)) {
    throw new Error(`K26G-T01 already landed on the current repo truth (${K26G_TASK.expectedChangedFiles[0]} already contains the trimmed numerology validation).`);
  }
  if (!currentContent.includes(K26G_TASK.expectedStrings.oldRequired) || !currentContent.includes(K26G_TASK.expectedStrings.oldBirthDate) || !currentContent.includes(K26G_TASK.expectedStrings.oldResult)) {
    throw new Error(`K26G-T01 cannot run because the expected numerology validation anchors are missing from ${K26G_TASK.expectedChangedFiles[0]}.`);
  }

  const invalidProbeUrl = `${baseUrl}/api/numerology/calc`;
  const preflightInvalidProbe = await fetchProbe(invalidProbeUrl, K26G_TASK.invalidBody, dispatcher);
  const preflightValidProbe = await fetchProbe(invalidProbeUrl, K26G_TASK.validBody, dispatcher);
  if (preflightInvalidProbe.status !== 200) {
    throw new Error(`K26G-T01 expected whitespace preflight probe to remain open on current runtime, got ${preflightInvalidProbe.status}.`);
  }
  if (preflightValidProbe.status !== 200) {
    throw new Error(`K26G-T01 expected valid preflight probe to succeed, got ${preflightValidProbe.status}.`);
  }
  ...
  const runtimeCommit = verifiedCommit
    ...
    : undefined;
  const postDeployInvalidProbe = await fetchProbe(invalidProbeUrl, K26G_TASK.invalidBody, dispatcher);
  const postDeployValidProbe = await fetchProbe(invalidProbeUrl, K26G_TASK.validBody, dispatcher);
  ...
  const invalidWhitespaceRejected = postDeployInvalidProbe.status === 400 && postDeployInvalidProbe.snippet.includes('profileId, name, birthDate, and system are required');
  ...
  if (taskClass !== K26G_TASK.expectedTaskClass) {
    assessmentReasons.push(`expected taskClass ${K26G_TASK.expectedTaskClass}, got ${taskClass}`);
  }
  ...
  if (JSON.stringify(changedFiles) !== JSON.stringify(K26G_TASK.expectedChangedFiles)) {
    assessmentReasons.push(`expected changedFiles ${K26G_TASK.expectedChangedFiles.join(', ')}, got ${changedFiles.join(', ') || 'none'}`);
  }
  ...
  if (!invalidWhitespaceRejected) {
    assessmentReasons.push(`expected whitespace probe to return 400 with required-field error, got ${postDeployInvalidProbe.status}`);
  }
  ...
  report.task = {
    taskId: K26G_TASK.taskId,
    title: K26G_TASK.title,
    status,
    ...
    verifiedCommit,
    changedFiles,
    scopeClean: changedFiles.every((filePath) => K26G_TASK.expectedChangedFiles.includes(filePath)),
