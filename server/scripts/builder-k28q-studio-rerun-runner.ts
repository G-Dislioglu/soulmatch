...
const K28Q_TASK = {
  taskId: 'K28Q-T01' as const,
  title: 'free class_1 oracle whitespace validation rerun with exact target lines',
  instruction:
    'In `server/src/routes/studio.ts`, change only the existing `/oracle` POST route. Add exactly the line `const question = typeof body.question === \'string\' ? body.question.trim() : \'\';` after the existing `const body = req.body as OracleRequestBody;` line. Change the validation to use `if (!question) {` while keeping the existing `400` payload `{ error: \'question required (purpose | soulperson | turning_point)\' }`. Replace the existing prompt line with exactly `const { system, user } = buildOraclePrompt(question as OracleQuestionType, body.profileExcerpt ?? \'\');` and replace the response line with exactly `res.json({ question, answer });`. Do not modify any other route, helper, provider logic, type, or file.',
...
function getDefaultOutputPath(repoRoot: string): string {
  return path.resolve(repoRoot, '..', 'k28q-studio-rerun-results.json');
}
...
async function main() {
  ...
  if (!token) throw new Error('OPUS_BRIDGE_SECRET missing for K2.8q runner');

  const baseUrl = (process.env.K28Q_BASE_URL ?? process.env.K28K_BASE_URL ?? process.env.K28J_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? 'https://soulmatch-1.onrender.com').replace(/\/$/, '');
  const resolveIp = process.env.K28Q_RESOLVE_IP
  ...
  const currentContent = normalizeText(await readRepoFile(repoRoot, K28Q_TASK.expectedChangedFiles[0]));
  if (K28Q_TASK.expectedStrings.landedPresent.every((entry) => currentContent.includes(entry))) {
    throw new Error(`K28Q-T01 already appears landed on current repo truth (${K28Q_TASK.expectedChangedFiles[0]} already contains all target markers).`);
  }
  for (const expectedPresent of K28Q_TASK.expectedStrings.baselinePresent) {
    if (!currentContent.includes(expectedPresent)) {
      throw new Error(`K28Q-T01 cannot run because expected baseline anchor is missing from ${K28Q_TASK.expectedChangedFiles[0]}.`);
    }
  }

  const routeUrl = `${baseUrl}/api/oracle`;
  const preflightInvalidProbe = await fetchProbe(routeUrl, K28Q_TASK.invalidBody, dispatcher);
  const preflightValidProbe = await fetchProbe(routeUrl, K28Q_TASK.validBody, dispatcher);

  if (preflightInvalidProbe.status !== 200) {
    throw new Error(`K28Q-T01 expected whitespace-only question preflight probe to remain open on current runtime, got ${preflightInvalidProbe.status}.`);
  }
  if (preflightValidProbe.status !== 200) {
    throw new Error(`K28Q-T01 expected valid preflight probe to succeed, got ${preflightValidProbe.status}.`);
  }
  ...
  const postDeployInvalidProbe = await fetchProbe(routeUrl, K28Q_TASK.invalidBody, dispatcher);
  const postDeployValidProbe = await fetchProbe(routeUrl, K28Q_TASK.validBody, dispatcher);
  ...
  if (taskClass !== K28Q_TASK.expectedTaskClass) assessmentReasons.push(`expected taskClass ${K28Q_TASK.expectedTaskClass}, got ${taskClass}`);
  ...
  if (JSON.stringify(changedFiles) !== JSON.stringify(K28Q_TASK.expectedChangedFiles)) {
    assessmentReasons.push(`expected changedFiles ${K28Q_TASK.expectedChangedFiles.join(', ')}, got ${changedFiles.join(', ') || 'none'}`);
  }
  ...
  report.task = {
    taskId: K28Q_TASK.taskId,
    title: K28Q_TASK.title,
    status,
    ...
    verifiedCommit,
    changedFiles,
    scopeClean: changedFiles.every((filePath) => K28Q_TASK.expectedChangedFiles.includes(filePath)),
