...
type PushBatchReport = {
  batch: 'K2.8s-OracleProviderGuard';
  runDate: string;
  baseUrl: string;
  resolveIp?: string;
  headBefore: string;
...
function getDefaultOutputPath(repoRoot: string): string {
  return path.resolve(repoRoot, '..', 'k28s-oracle-provider-guard-results.json');
}
...
async function main() {
  ...
  if (!token) throw new Error('OPUS_BRIDGE_SECRET missing for K2.8s runner');

  const baseUrl = (process.env.K28S_BASE_URL ?? process.env.K28Q_BASE_URL ?? process.env.K28K_BASE_URL ?? process.env.K28J_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? 'https://soulmatch-1.onrender.com').replace(/\/$/, '');
  const resolveIp = process.env.K28S_RESOLVE_IP
  ...
  const currentContent = normalizeText(await readRepoFile(repoRoot, K28S_TASK.expectedChangedFiles[0]));
  if (K28S_TASK.expectedStrings.landedPresent.every((entry) => currentContent.includes(entry))) {
    throw new Error(`K28S-T01 already appears landed on current repo truth (${K28S_TASK.expectedChangedFiles[0]} already contains all target markers).`);
  }
  for (const expectedPresent of K28S_TASK.expectedStrings.baselinePresent) {
    if (!currentContent.includes(expectedPresent)) {
      throw new Error(`K28S-T01 cannot run because expected baseline anchor is missing from ${K28S_TASK.expectedChangedFiles[0]}.`);
    }
  }

  const routeUrl = `${baseUrl}/api/oracle`;
  const preflightWhitespaceProbe = await fetchProbe(routeUrl, K28S_TASK.invalidWhitespaceProviderBody, dispatcher);
  const preflightUnknownProviderProbe = await fetchProbe(routeUrl, K28S_TASK.invalidUnknownProviderBody, dispatcher);
  const preflightValidProbe = await fetchProbe(routeUrl, K28S_TASK.validBody, dispatcher);

  if (preflightWhitespaceProbe.status !== 502) {
    throw new Error(`K28S-T01 expected whitespace-only provider preflight probe to fail on current runtime with 502, got ${preflightWhitespaceProbe.status}.`);
  }
  if (preflightUnknownProviderProbe.status !== 502) {
    throw new Error(`K28S-T01 expected unknown provider preflight probe to fail on current runtime with 502, got ${preflightUnknownProviderProbe.status}.`);
  }
  ...
  const postDeployWhitespaceProbe = await fetchProbe(routeUrl, K28S_TASK.invalidWhitespaceProviderBody, dispatcher);
  const postDeployUnknownProviderProbe = await fetchProbe(routeUrl, K28S_TASK.invalidUnknownProviderBody, dispatcher);
  const postDeployValidProbe = await fetchProbe(routeUrl, K28S_TASK.validBody, dispatcher);
  ...
  if (taskClass !== K28S_TASK.expectedTaskClass) assessmentReasons.push(`expected taskClass ${K28S_TASK.expectedTaskClass}, got ${taskClass}`);
  ...
  if (JSON.stringify(changedFiles) !== JSON.stringify(K28S_TASK.expectedChangedFiles)) {
    assessmentReasons.push(`expected changedFiles ${K28S_TASK.expectedChangedFiles.join(', ')}, got ${changedFiles.join(', ') || 'none'}`);
  }
  ...
  if (!validProbeBehaviorPreserved) {
    assessmentReasons.push(`expected valid probe status to remain ${preflightValidProbe.status}, got ${postDeployValidProbe.status}`);
  }
  ...
  report.task = {
    taskId: K28S_TASK.taskId,
    title: K28S_TASK.title,
    status,
    ...
    verifiedCommit,
    changedFiles,
    scopeClean: changedFiles.every((filePath) => K28S_TASK.expectedChangedFiles.includes(filePath)),
