...
type BatchReport = {
  batch: 'K2.8b-FreeClass1OpsSubset';
  runDate: string;
  baseUrl: string;
  resolveIp?: string;
  headBefore: string;
...
const TASKS: TaskConfig[] = [
  {
    taskId: 'K28B-T01',
    title: 'free append on tiny helper file inside released class_1 corridor',
    instruction:
      'Append exactly one new line `K2.8b tiny helper append smoke` to `docs/archive/k26c-helper-smoke.txt`. Do not modify any other line or file.',
    scope: ['docs/archive/k26c-helper-smoke.txt'],
    expectedChangedFiles: ['docs/archive/k26c-helper-smoke.txt'],
    expectedNeedle: 'K2.8b tiny helper append smoke',
  },
  {
    taskId: 'K28B-T02',
    title: 'free one-line helper create-target inside released class_1 corridor',
    instruction:
      'Create exactly one new file `docs/archive/k28b-free-class1-ops-smoke.txt` with exactly one line: `K2.8b free class_1 ops single-line helper smoke`. Do not modify any other file.',
    scope: ['docs/archive/k28b-free-class1-ops-smoke.txt'],
    expectedChangedFiles: ['docs/archive/k28b-free-class1-ops-smoke.txt'],
    expectedNeedle: 'K2.8b free class_1 ops single-line helper smoke',
  },
];
...
function getDefaultOutputPath(repoRoot: string): string {
  return path.resolve(repoRoot, '..', 'k28b-free-ops-results.json');
}
...
async function main() {
  ...
  if (!token) throw new Error('OPUS_BRIDGE_SECRET missing for K2.8b runner');

  const baseUrl = (process.env.K28B_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? 'https://soulmatch-1.onrender.com').replace(/\/$/, '');
  const resolveIp = process.env.K28B_RESOLVE_IP ?? process.env.K28A_RESOLVE_IP ?? process.env.K27F_RESOLVE_IP ?? process.env.K27E_RESOLVE_IP ?? process.env.K27D_RESOLVE_IP ?? process.env.K27C_RESOLVE_IP ?? process.env.DEPLOY_RESOLVE_IP;
  ...
  console.log(`[k28b-free] remote main before: ${headBefore}`);
