...
type PushBatchReport = {
  batch: 'K2.6e-CodeAdjacentControlledPush';
  runDate: string;
  baseUrl: string;
  resolveIp?: string;
  headBefore: string;
...
const K26E_TASK = {
  taskId: 'K26E-T01' as const,
  title: 'controlled class_1 health log cleanup push',
  instruction:
    'In `server/src/routes/health.ts`, replace exactly `console.log(\'ğŸ¥ Health endpoint hit - sweph test running...\');` with `console.log(\'[health] endpoint hit - sweph test running...\');` and replace exactly ``console.log(`ğŸ¥ Swiss Ephemeris available: ${swephProbe.available}`);`` with ``console.log(`[health] Swiss Ephemeris available: ${swephProbe.available}`);``. Do not modify any other file.',
  scope: ['server/src/routes/health.ts'],
  expectedTaskClass: 'class_1' as const,
  expectedChangedFiles: ['server/src/routes/health.ts'],
  expectedStrings: {
    oldA: "console.log('🏥 Health endpoint hit - sweph test running...');",
    newA: "console.log('[health] endpoint hit - sweph test running...');",
    oldB: "console.log(`🏥 Swiss Ephemeris available: ${swephProbe.available}`);",
    newB: "console.log(`[health] Swiss Ephemeris available: ${swephProbe.available}`);",
  },
};
...
function getDefaultOutputPath(repoRoot: string): string {
  return path.resolve(repoRoot, '..', 'k26e-push-results.json');
}
...
async function main() {
  ...
  if (!token) {
    throw new Error('OPUS_BRIDGE_SECRET missing for K2.6e runner');
  }

  const baseUrl = (process.env.K26E_BASE_URL ?? process.env.K26D_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? 'https://soulmatch-1.onrender.com').replace(/\/$/, '');
  const resolveIp = process.env.K26E_RESOLVE_IP ?? process.env.K26D_RESOLVE_IP ?? process.env.DEPLOY_RESOLVE_IP;
  ...
  const currentContent = await readRepoFile(repoRoot, K26E_TASK.expectedChangedFiles[0]);
  if (currentContent.includes(K26E_TASK.expectedStrings.newA) || currentContent.includes(K26E_TASK.expectedStrings.newB)) {
    throw new Error(`K26E-T01 already landed on the current repo truth (${K26E_TASK.expectedChangedFiles[0]} already contains the cleaned health logs).`);
  }
  if (!currentContent.includes(K26E_TASK.expectedStrings.oldA) || !currentContent.includes(K26E_TASK.expectedStrings.oldB)) {
    throw new Error(`K26E-T01 cannot run because the expected mojibake health log anchors are missing from ${K26E_TASK.expectedChangedFiles[0]}.`);
  }
  ...
  const report: PushBatchReport = {
    ...
    environment: {
      outputFile,
      task: K26E_TASK.taskId,
    },
  };

  console.log(`[k26e-push] remote main before: ${headBefore}`);
  ...
  const result = await fetchJson(`${baseUrl}/api/builder/opus-bridge/opus-task?opus_token=${encodeURIComponent(token)}`, dispatcher, {
    ...
    body: JSON.stringify({
      instruction: K26E_TASK.instruction,
      dryRun: false,
      skipDeploy: false,
      skipInlinePostPushChecks: false,
      scope: K26E_TASK.scope,
  ...
  if (taskClass !== K26E_TASK.expectedTaskClass) {
    assessmentReasons.push(`expected taskClass ${K26E_TASK.expectedTaskClass}, got ${taskClass}`);
  }
  ...
  if (JSON.stringify(changedFiles) !== JSON.stringify(K26E_TASK.expectedChangedFiles)) {
    assessmentReasons.push(`expected changedFiles ${K26E_TASK.expectedChangedFiles.join(', ')}, got ${changedFiles.join(', ') || 'none'}`);
  }
  ...
  report.task = {
    taskId: K26E_TASK.taskId,
    title: K26E_TASK.title,
    status,
    ...
    verifiedCommit,
    changedFiles,
    scopeClean: changedFiles.every((filePath) => K26E_TASK.expectedChangedFiles.includes(filePath)),
