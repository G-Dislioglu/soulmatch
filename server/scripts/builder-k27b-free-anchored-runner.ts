...
type RunReport = {
  batch: 'K2.7b-FreeAnchoredReplacement';
  runDate: string;
  baseUrl: string;
  resolveIp?: string;
  headBefore: string;
...
const TARGET_FILE = 'docs/archive/k26c-helper-smoke.txt';
const SEARCH = 'K2.6c create-target tiny helper smoke';
const REPLACE = 'K2.7b anchored helper replacement smoke';
...
function getDefaultOutputPath(repoRoot: string): string {
  return path.resolve(repoRoot, '..', 'k27b-free-anchored-results.json');
}
...
async function main() {
  ...
  if (!token) {
    throw new Error('OPUS_BRIDGE_SECRET missing for K2.7b runner');
  }

  const baseUrl = (process.env.K27B_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? 'https://soulmatch-1.onrender.com').replace(/\/$/, '');
  const resolveIp = process.env.K27B_RESOLVE_IP ?? process.env.DEPLOY_RESOLVE_IP;
  ...
  console.log(`[k27b-free] remote main before: ${headBefore}`);
