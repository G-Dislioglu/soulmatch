...
const K27F_TASK = {
  taskId: 'K27F-T01' as const,
  title: 'free class_1 scoring profileId whitespace validation push',
  instruction:
    'In `server/src/routes/scoring.ts`, tighten only the existing `/calc` route so a missing or whitespace-only `profileId` is rejected before calculation. Add a local trimmed `profileId` variable inside the route handler, keep the existing 400 error message `profileId, numerologyA, and numerologyB are required` when that trimmed value is empty, and otherwise pass the trimmed `profileId` into `calculateScore(...)` without changing any other route, helper, type, or file.',
  scope: ['server/src/routes/scoring.ts'],
  expectedTaskClass: 'class_1' as const,
  expectedChangedFiles: ['server/src/routes/scoring.ts'],
  invalidBody: '{"profileId":"   ","numerologyA":{"numbers":{"lifePath":1,"expression":2,"soulUrge":3,"personality":4,"birthday":5}},"numerologyB":{"numbers":{"lifePath":2,"expression":3,"soulUrge":4,"personality":5,"birthday":6}}}',
  validBody: '{"profileId":"abc","numerologyA":{"numbers":{"lifePath":1,"expression":2,"soulUrge":3,"personality":4,"birthday":5}},"numerologyB":{"numbers":{"lifePath":2,"expression":3,"soulUrge":4,"personality":5,"birthday":6}}}',
  expectedStrings: {
    routeStart: "scoringRouter.post('/calc', (req: Request, res: Response) => {",
    oldRequest: '    const request: UnifiedScoringRequest = req.body;',
    oldValidation: '    if (!request.profileId || !request.numerologyA || !request.numerologyB) {',
    oldResult: '    const result = calculateScore(request);',
    newTrimProfile: "    const profileId = typeof request.profileId === 'string' ? request.profileId.trim() : '';",
    newCalculate: '    const result = calculateScore({',
  },
};
...
function getDefaultOutputPath(repoRoot: string): string {
  return path.resolve(repoRoot, '..', 'k27f-free-runtime-results.json');
}
...
async function main() {
  ...
  if (!token) {
    throw new Error('OPUS_BRIDGE_SECRET missing for K2.7f runner');
  }

  const baseUrl = (process.env.K27F_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? 'https://soulmatch-1.onrender.com').replace(/\/$/, '');
  const resolveIp = process.env.K27F_RESOLVE_IP ?? process.env.K27E_RESOLVE_IP ?? process.env.K27D_RESOLVE_IP ?? process.env.K27C_RESOLVE_IP ?? process.env.DEPLOY_RESOLVE_IP;
  const dispatcher = createResolveDispatcher(baseUrl, resolveIp);
  const outputFile = getDefaultOutputPath(repoRoot);

  const currentContent = normalizeText(await readRepoFile(repoRoot, K27F_TASK.expectedChangedFiles[0]));
  if (currentContent.includes(K27F_TASK.expectedStrings.newTrimProfile)) {
    throw new Error(`K27F-T01 already landed on the current repo truth (${K27F_TASK.expectedChangedFiles[0]} already contains trimmed scoring profileId validation).`);
  }
  if (
    !currentContent.includes(K27F_TASK.expectedStrings.routeStart)
    || !currentContent.includes(K27F_TASK.expectedStrings.oldRequest)
    || !currentContent.includes(K27F_TASK.expectedStrings.oldValidation)
    || !currentContent.includes(K27F_TASK.expectedStrings.oldResult)
  ) {
    throw new Error(`K27F-T01 cannot run because the expected scoring route anchors are missing from ${K27F_TASK.expectedChangedFiles[0]}.`);
  }

  const calcUrl = `${baseUrl}/api/scoring/calc`;
  const preflightInvalidProbe = await fetchProbe(calcUrl, K27F_TASK.invalidBody, dispatcher);
  const preflightValidProbe = await fetchProbe(calcUrl, K27F_TASK.validBody, dispatcher);
  if (preflightInvalidProbe.status !== 200) {
    throw new Error(`K27F-T01 expected whitespace-only profileId preflight probe to remain open on current runtime, got ${preflightInvalidProbe.status}.`);
  }
  if (preflightValidProbe.status !== 200) {
    throw new Error(`K27F-T01 expected valid preflight probe to succeed, got ${preflightValidProbe.status}.`);
  }
  ...
  const postDeployInvalidProbe = await fetchProbe(calcUrl, K27F_TASK.invalidBody, dispatcher);
  const postDeployValidProbe = await fetchProbe(calcUrl, K27F_TASK.validBody, dispatcher);
  ...
  const invalidWhitespaceRejected = postDeployInvalidProbe.status === 400 && postDeployInvalidProbe.snippet.includes('profileId, numerologyA, and numerologyB are required');
  ...
  if (taskClass !== K27F_TASK.expectedTaskClass) assessmentReasons.push(`expected taskClass ${K27F_TASK.expectedTaskClass}, got ${taskClass}`);
  ...
  if (JSON.stringify(changedFiles) !== JSON.stringify(K27F_TASK.expectedChangedFiles)) {
    assessmentReasons.push(`expected changedFiles ${K27F_TASK.expectedChangedFiles.join(', ')}, got ${changedFiles.join(', ') || 'none'}`);
  }
  ...
  if (!invalidWhitespaceRejected) assessmentReasons.push(`expected whitespace-only profileId probe to return 400, got ${postDeployInvalidProbe.status}`);
  if (!validProbeStillOk) assessmentReasons.push(`expected valid probe to remain 200 with stable profileId, got ${postDeployValidProbe.status}`);
  ...
  report.task = {
    taskId: K27F_TASK.taskId,
    title: K27F_TASK.title,
    status,
    ...
    verifiedCommit,
    changedFiles,
    scopeClean: changedFiles.every((filePath) => K27F_TASK.expectedChangedFiles.includes(filePath)),
