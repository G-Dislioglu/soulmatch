...
const K28K_TASK = {
  taskId: 'K28K-T01' as const,
  title: 'free class_1 oracle question whitespace validation push',
  instruction:
    'In `server/src/routes/studio.ts`, tighten only the existing `/oracle` POST route so a whitespace-only `question` is rejected before prompt construction. Add a local trimmed `question` variable inside that route handler, keep the existing `400` payload `{ error: \'question required (purpose | soulperson | turning_point)\' }` when the trimmed value is empty, and use the trimmed value for `buildOraclePrompt(...)` and the response payload. Do not change any other route, helper, provider logic, type, or file.',
  ...
  expectedStrings: {
    routeStart: "studioRouter.post('/oracle', async (req: Request, res: Response) => {",
    oldValidation: '  if (!body.question) {',
    oldPrompt: "  const { system, user } = buildOraclePrompt(body.question, body.profileExcerpt ?? '');",
    oldResponse: '    res.json({ question: body.question, answer });',
    newTrimmedQuestion: "  const question = typeof body.question === 'string' ? body.question.trim() : '';",
    newPrompt: "  const { system, user } = buildOraclePrompt(question as OracleQuestionType, body.profileExcerpt ?? '');",
    newResponse: '    res.json({ question, answer });',
  },
};
...
function getDefaultOutputPath(repoRoot: string): string {
  return path.resolve(repoRoot, '..', 'k28k-free-runtime-results.json');
}
...
async function main() {
  ...
  if (!token) throw new Error('OPUS_BRIDGE_SECRET missing for K2.8k runner');

  const baseUrl = (process.env.K28K_BASE_URL ?? process.env.K28J_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? 'https://soulmatch-1.onrender.com').replace(/\/$/, '');
  const resolveIp = process.env.K28K_RESOLVE_IP
  ...
  const currentContent = normalizeText(await readRepoFile(repoRoot, K28K_TASK.expectedChangedFiles[0]));
  if (currentContent.includes(K28K_TASK.expectedStrings.newTrimmedQuestion)) {
    throw new Error(`K28K-T01 already landed on the current repo truth (${K28K_TASK.expectedChangedFiles[0]} already contains trimmed oracle question validation).`);
  }
  if (
    !currentContent.includes(K28K_TASK.expectedStrings.routeStart)
    || !currentContent.includes(K28K_TASK.expectedStrings.oldValidation)
    || !currentContent.includes(K28K_TASK.expectedStrings.oldPrompt)
    || !currentContent.includes(K28K_TASK.expectedStrings.oldResponse)
  ) {
    throw new Error(`K28K-T01 cannot run because the expected oracle route anchors are missing from ${K28K_TASK.expectedChangedFiles[0]}.`);
  }

  const routeUrl = `${baseUrl}/api/oracle`;
  const preflightInvalidProbe = await fetchProbe(routeUrl, K28K_TASK.invalidBody, dispatcher);
  const preflightValidProbe = await fetchProbe(routeUrl, K28K_TASK.validBody, dispatcher);

  if (preflightInvalidProbe.status !== 200) {
    throw new Error(`K28K-T01 expected whitespace-only question preflight probe to remain open on current runtime, got ${preflightInvalidProbe.status}.`);
  }
  if (preflightValidProbe.status !== 200) {
    throw new Error(`K28K-T01 expected valid preflight probe to succeed, got ${preflightValidProbe.status}.`);
  }
  ...
  const postDeployInvalidProbe = await fetchProbe(routeUrl, K28K_TASK.invalidBody, dispatcher);
  const postDeployValidProbe = await fetchProbe(routeUrl, K28K_TASK.validBody, dispatcher);
  ...
  const invalidWhitespaceRejected = postDeployInvalidProbe.status === 400 && postDeployInvalidProbe.snippet.includes('"error":"question required (purpose | soulperson | turning_point)"');
  const validProbeStillOk = postDeployValidProbe.status === 200 && postDeployValidProbe.snippet.includes('"question":"purpose"');

  if (status !== 'success') assessmentReasons.push(`expected status success, got ${status}`);
  if (taskClass !== K28K_TASK.expectedTaskClass) assessmentReasons.push(`expected taskClass ${K28K_TASK.expectedTaskClass}, got ${taskClass}`);
  ...
  if (JSON.stringify(changedFiles) !== JSON.stringify(K28K_TASK.expectedChangedFiles)) {
    assessmentReasons.push(`expected changedFiles ${K28K_TASK.expectedChangedFiles.join(', ')}, got ${changedFiles.join(', ') || 'none'}`);
  }
  ...
  if (!invalidWhitespaceRejected) assessmentReasons.push(`expected whitespace-only question probe to return 400, got ${postDeployInvalidProbe.status}`);
  ...
  report.task = {
    taskId: K28K_TASK.taskId,
    title: K28K_TASK.title,
    status,
    ...
    verifiedCommit,
    changedFiles,
    scopeClean: changedFiles.every((filePath) => K28K_TASK.expectedChangedFiles.includes(filePath)),
