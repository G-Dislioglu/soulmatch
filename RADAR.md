...
- `risk`: `niedrig bis mittel; der Hardening-Zwischenschritt ist live in den Phase-1-Pfad eingelassen, und 910fbed reduziert die groben Prosa-False-Positives ueber code-block-aware Matching. Offen bleibt nicht die Existenz des Guards, sondern seine Restpraezision an weiteren Randmustern und die Beobachtungstiefe der Findings.`
...
- `kurzurteil`: `Der vorgeschaltete Hardening-Block ist umgesetzt und live verifiziert: Builder-Instruktionen werden vor dem Dispatch deterministisch gehaertet, vergiftete Assumptions blocken vor dem Swarm, der Regex-False-Positive wurde mit 4a072ec nachgeschaerft, und 910fbed macht die zentralen Forbidden-Patterns kontextbewusst, sodass Prosa mit Triggerwoertern nicht mehr stumpf als Injection gelesen wird.`
- `evidence`: `65e705d` fuehrt den Hardening-Pfad repo-sichtbar ein; `47deb25` baut Phase 1 mit AICOS-Meta-Loader, Assumption-Registry, Dispatch-Gate und `/api/architect/state`; `4a072ec` korrigiert einen produktiv nachgewiesenen False-Positive im Exfiltration-Regex; `910fbed` macht die wichtigsten Forbidden-Patterns in `server/src/lib/specHardening.ts` code-block-aware. Live-Proben 2026-04-24/25: Render-Deploys auf `47deb25`, `4a072ec` und `910fbed` verifiziert, produktive Migration von `builder_assumptions` ueber `/api/builder/opus-bridge/migrate`, `/api/architect/state` zeigt `persistenceMode=database`, der Async-Gate-Smoke `job-mod99ozd` blockt die vergiftete Assumption vor dem Worker-Swarm bei gleichzeitig akzeptierten Meta-Karten `meta-001` bis `meta-007`, und ein weiterer Async-Dry-Run mit prose-eingebetteten Triggerwoertern wurde live akzeptiert statt vom Hardening-Gate geblockt.`
