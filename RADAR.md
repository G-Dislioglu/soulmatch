...
- `why_not_now`: `Der freie Docs-/Helper-class_1-Korridor ist jetzt nicht nur ueber K2.7a bis K2.7c belastbar, sondern durch `K2.8b` und `K2.8c` auch in einem echten engen Operations- plus Runtime-Subset gruen. Lokal zeigt `K2.8d` zusaetzlich, dass die beiden offenen K2.8a-Formen am Judge-/Snapshot-Evidence-Pfad hingen. Am 2026-04-30 divergierten identische Live-T03-Dry-Runs auf demselben unveraenderten Head `385cf22` bereits zwischen altem Preview-Block und erneutem Approve (`K2.8f`). Gerade deshalb ist jetzt nicht der Moment fuer breite freie Builder-Autonomie oder weitere gleiche Vergleichslaeufe, sondern fuer das gezielte repo-/deploy-sichtbare Landen dieses schmalen Hardening-Schnitts.`
...
- `risk`: weiter reduziert, aber nicht null. Die aktuelle Kette blockiert jetzt mehrdeutige Search-Anker, Null-Diffs, falsche Datei-Claims und Markdown-Patches im falschen Txx-Abschnitt; kontrollierte Runs koennen Side-Effects inzwischen ueber `mode=none` unterdruecken, H2B fuegt vor dem Push einen konservativen Workflow-Simulations-Hook ein, H2C-1 macht diesen Pfad im kanonischen Orchestrator-Result verstaendlicher, H2D-1 zieht vorhandene Evidence-/Schema-Lock-Signale in einen expliziten Analysis-Layer zusammen, H2C-2 traegt dieselbe Wahrheit additiv auch in den Legacy-`/build`-Pfad, H3A behandelt den reinen Callback-Timeout jetzt nur noch als pending truth statt als harte Negativ-Aussage und H3-async-0/H3-async-1 ziehen fuer den Async-Health-Pfad die spaete Callback-Wahrheit jetzt bis in `async_jobs.result` nach. `1761f3e`, `4e4c72b` und `1272ccd` haerten danach fail-closed Scope-Klassifikation, degraded-provider-Verhalten und die Judge-Lane auf `main`; `96fc618` fuehrt den K2.6b-Live-Runner ein; `9f978e6` engt degraded-state auf Provider+Model ein. Mit `0738700`, `df13183`, `3b0236a`, `427235a`, `52a7175`, `adc593a` und `abd1d3f` ist ein enger Docs-/Helper- plus code-adjacent Cleanup-/Guard-/Validation-Korridor repo- und live-sichtbar gruen. `7c3bce7`, `21d7a3d`, `0b2b8c3`, `ef100dd`, `1512f95`, `b6b85d0`, `0702adc` und jetzt `385cf22` belegen den freien Korridor technisch und operativ belastbar; `K2.8a` zeigte danach die operative Grenze fuer groesseren Docs-Append und mehrzeilige Create-Targets, waehrend `K2.8b` tiny-helper-append und single-line helper create-target sauber gruen zog und `K2.8c` den ersten echten stateless public-route Runtime-Free-Run in `journey.ts` repo- und live-sichtbar nachzog. Lokal reduziert `K2.8d` das Rest-Risiko fuer genau diese beiden Formen weiter: der Evidence-Pfad in Judge und Snapshot ist gehaertet und die beiden Faelle laufen im lokalen Orchestrator gruen. Offen bleibt bewusst die repo-/live-seitige Bestaetigung dieses lokalen Hardening-Schnitts; kein Multi-File, kein breiter Builder-Core, keine Secrets-/Deploy-/Provider-Core-Ziele und weiterhin keine freie Korridor-Widening ohne neuen Nachweis.`
...
- `kurzurteil`: Der Acceptance-Korridor ist jetzt nicht mehr nur evidenzstark, sondern in einer engen Form operativ nutzbar. Der schmale K2.6b-Live-Subset blieb auf matching Live-Heads `96fc618` und `9f978e6` gruen 5/5; K2.6c landete komplett gruen im Docs-/Helper-Corridor; `427235a`, `52a7175`, `adc593a` und `abd1d3f` fuegten danach echte code-adjacent Cleanup-/Guard-/Validation-Faelle hinzu. Mit `7c3bce7`, `21d7a3d`, `0b2b8c3` und `ef100dd` ist der freie Docs-/Helper-class_1-Korridor inklusive non-governance Anchored-Replacements repo-sichtbar belegt. `1512f95`, `b6b85d0`, `0702adc` und `385cf22` erweitern diese Linie in vier freie Runtime-Faelle. `K2.8a` zeigte die ehrliche operative Grenze, `K2.8b` den gruenen engeren Ops-Schnitt, `K2.8d` lokalisiert die Ursache im Evidence-Pfad, und der T03-Recheck vom 2026-04-30 zeigt jetzt noch schmaler, dass die Restoffenheit nicht ein besonderer non-dry-Zweig ist, sondern schon auf dem alten Live-Head oszillierende Judge-Evidence zwischen Block und Approve. Offen bleibt damit primaer das repo-/deploy-sichtbare Landen dieses engen Hardening-Schnitts.
...
- `evidence_addendum`: Repo-sichtiger K2.8c-Free-Ops-Runtime-Report in `docs/BUILDER-BENCHMARK-K2.8C-FREE-OPS-RUNTIME-REPORT.md`; `385cf2261a9dcfb35e5158c82dadb585a891d7c9` landete den Journey-Validation-Fix auf `server/src/routes/journey.ts`, whitespace-only `startDate` kippte live von `200` auf `400 invalid_request`, der gueltige Kontrollprobe-Body blieb `200`, und `/api/health` zog danach auf `385cf2261a9dcfb35e5158c82dadb585a891d7c9` nach. Lokaler K2.8d-Hardening-Report in `docs/BUILDER-BENCHMARK-K2.8D-LOCAL-JUDGE-HARDENING-REPORT.md`; `pnpm --dir server builder:k28d` lief 2/2 gruen auf der `grok`-Lane, und die beiden frueheren K2.8a-Fail-Closed-Formen kamen im lokalen Orchestrator als `decision=approve` und `judgeStatus=ok` durch, bewusst ohne Live- oder `main`-Behauptung. Am 2026-04-30 divergierten danach identische Live-T03-Dry-Runs gegen denselben Runtime-Head `385cf22`: ein direkter Recheck blockte wieder mit `judgeReason`: `previews show single-line content without newlines`, waehrend der repo-sichtige `K2.8f`-Runner denselben Fall wieder als `dry_run` mit `decision=approve` durchliess. Damit ist die Restfragilitaet auf dem ungehaerteten Live-Head als echte Run-to-Run-Oszillation belegt.
