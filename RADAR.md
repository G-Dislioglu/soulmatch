...
- `next_gate`: `proposal`
- `why_not_now`: `Die Approval-Frage ist durch `K2.8o` und `K2.8p` jetzt live beantwortet. Offen bleibt damit nicht mehr "ob class_2 geht", sondern der engere Rest auf der grossen `studio.ts`-Landing-Lane.`
...
- `kurzurteil`: Der Acceptance-Korridor bleibt fuer die bereits belegten single-file `class_1` docs/helper- und einige enge Runtime-Faelle belastbar. `K2.8o` und `K2.8p` zeigen jetzt zusaetzlich, dass der approval-backed `class_2`-Pfad operativ funktioniert. Der verbleibende konkrete Rest ist enger: grosse `studio.ts`-Targets bleiben landing-fragil, waehrend Approval selbst nicht mehr die Hauptunsicherheit ist.`
