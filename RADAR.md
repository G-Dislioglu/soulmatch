...
- `why_not_now`: `Der breite Builder-Autonomie-Korridor braucht jetzt keinen neuen Patrol- oder UI-Ausbau. K2.6a Batch 1 ist lokal gruen, K2.6b wurde ueber den echten Live-HTTP-Pfad gefahren, und der verbleibende Schritt ist jetzt nicht neuer Featurebau, sondern Live-Sync der aktuellen Branch-Semantik.`
...
- `risk`: weiter reduziert, aber nicht null. Die K2.6a-Kette ist lokal bis Batch 1 belastbar: class_1, class_2-missing-approval, class_3-manual-only und Ambiguitaetsfaelle laufen ohne Deviation durch. K2.6b zeigt jetzt, dass der Runner selbst und der DNS-/Resolve-Pfad tragfaehig sind, aber Live `ed27349` bei `T08` und `T10` noch hinter der Branch-Semantik liegt. Das reale Restthema ist damit zweigeteilt: weiter Provider-Unabhaengigkeit auf der Worker-Seite und vor allem Live-Lag gegen die aktuelle Branch-Haertung.
- `betroffene_bereiche`: `server/src/lib/builderSafetyPolicy.ts`, `server/src/lib/opusTaskOrchestrator.ts`, `server/src/lib/opusJudge.ts`, `server/src/lib/opusEnvelopeValidator.ts`, `server/scripts/builder-k26a-runner.ts`, `server/scripts/builder-k26b-live-runner.ts`, `docs/BUILDER-BENCHMARK-K2.6A-RUNNER-PREFLIGHT.md`, `docs/BUILDER-BENCHMARK-K2.6A-EXECUTION-PLAN.md`, `docs/BUILDER-BENCHMARK-K2.6A-BATCH1-REPORT.md`, `docs/BUILDER-BENCHMARK-K2.6B-LIVE-SUBSET-REPORT.md`.
- `kurzurteil`: Der breite Acceptance-Korridor hat jetzt nicht nur einen sauberen lokalen Benchmark-Beleg, sondern auch einen echten Live-Subset-Runner. K2.6a Batch 1 lief nach `73cf30b` mit 8/8 Passes durch; K2.6b auf Live `ed27349` zeigte danach 3/5 Passes und isolierte die verbleibenden Abweichungen als Live-Lag auf `class_3`-Preflight und Ambiguitaets-Fail-Closed. Der naechste Builder-Restpunkt ist damit enger als zuvor: nicht neue Patrol-/Architect-Arbeit, sondern Branch-Semantik auf Live ziehen und denselben K2.6b-Subset erneut fahren.
- `evidence`: Lokale Approval-Pair-Evidence `k26a-t06-result-1777261691167.json` und `k26a-t07-result-1777261744811.json` mit T06 `approval-validation=ok`, T07 `requiredExternalApproval=true` und `pushBlockedReason=class_2 requires approved plan + approvalId before live push.`; Cleanup-Evidence fuer das Test-Approval ueber `k26a-approval-cleanup.ts` mit `beforeFound=true`, `deleted=true`, `afterCount=0`; nicht akzeptierte Landing-Evidence `class1-builder-run-k26-t03-docfix-after-wiring-20260427-230936.json`; Runner-Commit `fc012b4`; Fail-closed-Hardening `ca4b55a`; degraded-provider hardening `7984db6`; resiliente Judge-Fallback-Lane plus repo-nah korrigierter T02-Corpus `73cf30b`; K2.6b-Live-Runner `1275c63`; aktueller Batch-1-Report extern in `C:/Users/guerc/OneDrive/Desktop/soulmatch/k26a-batch1-results.json` und repo-sichtig in `docs/BUILDER-BENCHMARK-K2.6A-BATCH1-REPORT.md` mit `totalTasks=8`, `passCount=8`, `deviationCount=0`, `providerDegraded=["gpt"]`, `judgeLane="grok"` fuer die betroffenen class_1/class_2-Laeufe. K2.6b liegt extern in `C:/Users/guerc/OneDrive/Desktop/soulmatch/k26b-live-results.json` und repo-sichtig in `docs/BUILDER-BENCHMARK-K2.6B-LIVE-SUBSET-REPORT.md`; Live `ed27349` erreichte `passCount=3/5`, waehrend lokale HTTP-Spot-Checks auf aktuellem Branch `T08` und `T10` sauber passieren liessen. Letzte live bestaetigte Runtime-Antwort bleibt weiterhin auf Code-Head `ed27349`.
...
- `next_gate`: `archive`
- `absorbed_into`: `client/src/modules/M16_builder/hooks/useBuilderApi.ts`, `client/src/modules/M16_builder/ui/PatrolConsole.tsx`, `STATE.md`, `FEATURES.md`, `RADAR.md`
- `why_not_now`: `none`
- `non_scope`: freie Routine-Modellkonfiguration, weitere Deep-Patrol-Arbeit, Server-Aenderungen, neue Patrol-Panels, Scheduling, Policy-Settings oder Patrol-Profiles.
- `risk`: reduziert; die grobe UI-/Contract-Drift ist branch-sichtig bereinigt, der kleine Rest liegt hoechstens noch in Success-/Error-/Loading-Politur.
...
- `kurzurteil`: Der schmale Ehrlichkeitsblock ist branch-sichtig gelandet: Routine-Slots sind read-only auf echte Server-Wahrheit gezogen, ein minimaler `Run Routine Patrol`-Trigger nutzt `POST /api/builder/opus-bridge/patrol-trigger-round`, und Patrol-Feed-Fehler werden sichtbar. Deep Patrol blieb dabei unberuehrt.
- `evidence`: `server/src/routes/opusBridge.ts` bietet `POST /patrol-trigger-round` ohne Request-Body sowie `GET /patrol-status` und `GET /patrol-findings`; `server/src/lib/scoutPatrol.ts` fuehrt feste `ROUTINE_MODELS`; `client/src/modules/M16_builder/ui/PatrolConsole.tsx` zeigt Routine-Scouts seit `332000c` read-only, triggert denselben Serverpfad minimal und surfacet den Feed-Fehler sichtbar; `client/src/modules/M16_builder/hooks/useBuilderApi.ts` traegt den body-losen Routine-Trigger branch-sichtig.
