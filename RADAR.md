...
- `kurzurteil`: Der Director ist jetzt nicht nur live faehig, sondern auch produktnah bedienbar: drei Modelle, Fast/Deep-Thinking und sichtbare Action-Badges sind repo-sichtbar gebaut. Der naechste enge Block bleibt trotzdem der Live-Nachweis fuer delegierte Async-Tasks samt Status und UI-Spur.
- `evidence`: `MayaDashboard.tsx` bietet jetzt ein Director-Dropdown fuer `Opus 4.6`, `GPT 5.4` und `GLM 5.1`, einen `Fast`/`Deep`-Schalter und Action-Badges unter Director-Antworten; `useMayaApi.ts` sendet `directorModel`, `thinking` und `conversationHistory`; `builder.ts` mappt diese drei Director-Modelle auf echte Provider-Konfigurationen; `providers.ts` reicht Thinking/Reasoning an Anthropic, OpenAI und OpenRouter durch; `opusWorkerSwarm.ts`, `opusWorkerRegistry.ts` und `poolState.ts` fuehren `GLM 5.1` als Meister-/Worker-Pfad.
