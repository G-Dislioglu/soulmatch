...
- `why_not_now`: `K2.6g ist jetzt ebenfalls live gruen bestaetigt. Was gerade nicht sinnvoll waere, ist aus K2.6c+d+e+f+g sofort breite freie Builder-Autonomie abzuleiten; der naechste sinnvolle Schritt ist jetzt eher eine explizite class_1-Freigabeentscheidung als reflexhaft noch ein weiterer Blindkorridor.`
...
- `kurzurteil`: Der Acceptance-Korridor ist jetzt enger und belastbarer belegt als vor dem Incident. Der schmale K2.6b-Live-Subset blieb auf matching Live-Heads `96fc618` und `9f978e6` gruen 5/5; K2.6c landete komplett gruen im Docs-/Helper-Corridor; `427235a` zeigte den ersten code-adjacent single-file Marker-Push; `52a7175` fuegte danach den ersten realen code-adjacent Operational-Cleanup-Push in einer Runtime-Datei hinzu; `adc593a` schloss den engen Korridor um einen echten Runtime-Guard-Fix; `abd1d3f` fuegte jetzt einen echten Input-Validation-Fix in einem oeffentlichen Runtime-Pfad hinzu. Offen bleibt damit weniger die Frage, ob Builder diesen engen Push-Stil tragen kann, sondern wie wir den freien `class_1`-Korridor bewusst formulieren, ohne wieder in breite Freigabe-Behauptungen zu kippen.
