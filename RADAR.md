...
- `next_gate`: `proposal`
- `why_not_now`: `Die fruehere K2.8a-Restoffenheit ist jetzt geschlossen. `7f95aac` landete das enge Preview-/Snapshot-Hardening, `88e2d5a` zog den mehrzeiligen helper create-target non-dry scope-clean nach, und `6e1ea41` zog den groesseren Docs-Append non-dry scope-clean nach. Damit gibt es in diesem konkreten Restblock keinen Pflicht-Follow-up mehr; weitere Builder-Arbeit sollte erst wieder als neuer enger Kandidat formuliert werden.`
...
- `kurzurteil`: Der Acceptance-Korridor ist jetzt nicht mehr nur evidenzstark, sondern fuer single-file class_1 docs/helper- und enge Runtime-Faelle operativ belastbar. `K2.8a` markierte die Grenze ehrlich, `K2.8d` lokalisierte die Ursache im Evidence-Pfad, `7f95aac` landete genau dieses Hardening, und `K2.8g` plus `K2.8h` zogen die beiden frueheren Restfaelle non-dry scope-clean nach. Damit ist dieser konkrete Restblock abgeschlossen; weitere Builder-Grenzarbeit braucht wieder einen neuen engen Kandidaten statt weiteres Nachstochern an derselben Frage.
