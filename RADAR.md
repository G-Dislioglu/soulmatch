...
- `next_gate`: `proposal`
- `why_not_now`: `Nach `K2.8j` wurden vier weitere enge Kandidaten geprueft. `K2.8k` blieb auf `studio.ts` an `patch-via-push` haengen; `K2.8l`, `K2.8m` und `K2.8n` zeigten echte Live-Gaps, wurden aber alle als `class_2` fail-closed auf `dry_run_only` gezogen. Es gibt deshalb gerade keinen offensichtlichen freien Folgekandidaten mehr, der nicht kuenstlich Scope verbreitert.`
...
- `kurzurteil`: Der Acceptance-Korridor bleibt fuer die bereits belegten single-file `class_1` docs/helper- und einige enge Runtime-Faelle belastbar, aber `K2.8k` bis `K2.8n` ziehen die neue Grenze jetzt ehrlicher: grosse `studio.ts`-Targets bleiben landing-fragil, DB-backed create routes widen auf `class_2`, und computation-driving Validation-Faelle widen derzeit ebenfalls auf `class_2`. Weitere freie Builder-Grenzarbeit braucht deshalb wieder einen sehr nah an der bestehenden Beweislinie liegenden Kandidaten oder bewusst einen separaten Approval-Track.`
