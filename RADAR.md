...
- `next_gate`: `proposal`
- `why_not_now`: `Die Docs-/Helper-Frage ist durch `K2.9a` beantwortet, und `K2.9b` hat die erste Fresh-Family-Grenze sichtbar gemacht. Offen bleibt nicht mehr "ob Builder frei im released lane arbeiten kann", sondern ob der naechste Messschritt bewusst approval-backed class_2 sein soll oder ob der offene `zimage`-Bug als normaler Produkt-Hotfix behandelt werden soll.`
...
- `kurzurteil`: Der Acceptance-Korridor ist jetzt dreifach geschaerft: historisch ueber die K2.6-K2.8-Linie, positiv ueber `K2.9a` im released docs/helper lane und negativ/aufschlussreich ueber `K2.9b` in einer frischen external-service Family. Builder kann im released narrow free `class_1` lane bereits als kontrollierter Worker genutzt werden; approval-backed `class_2` ist separat belegt; frische provider-/cost-backed image-generation routes widen derzeit weiter auf `class_2`.`
