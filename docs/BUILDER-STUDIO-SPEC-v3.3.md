...
> **Umsetzungs-Stand (Stichtag S35-F9, 2026-04-20):** Weitgehend produktiv auf `main`. Live: Reuse-First-Prinzip (R1/R2/R3), Builder-UI unter `/builder`, Pool-System (Maya/Council/Worker/Scout/Destillierer), Agent-Habitat mit `builder_agent_profiles`-Tabelle, Canary-Stage-System, atomare Mehrdatei-Commits via Git Data API, Session-Log-Endpoint mit SHA-Backfill (S34), Pool-Config-Persistenz (F7), **F9-Pipeline-Honesty live** (Commit `1065cd3`: Callback-basierter Landungs-Verify in `opusSmartPush.ts` via neues Modul `pushResultWaiter.ts`, `/api/builder/tasks/:id/execution-result`-Handler kennt `committed === false`-Branch mit `reason`-Feld, Orchestrator-Status-Treue automatisch mitgezogen). Offen: F9 Schritt C (Workflow-Härtung `builder-executor.yml` — empty-diff sendet Callback statt stillem `exit 0`), blockiert durch fehlenden `workflows`-Scope im Bridge-Token (Drift 12), manueller Commit via Web-UI steht aus. Diese Spec bleibt als Vision-Dokument stehen; der tatsächliche Code-Stand liegt in `STATE.md` und den Handoffs S24–S35.
