...
- `next_recommended_block`: `Der naechste sinnvolle Builder-Block ist jetzt kein weiterer Approval-Nachweis mehr, sondern die Isolierung der verbleibenden `studio.ts`-Landing-Fragilitaet. `K2.8o` und `K2.8p` haben die approval-backed class_2-Lane live bewiesen; `K2.8q` zeigt, dass `/api/oracle` in `server/src/routes/studio.ts` weiter vor Commit an `patch-via-push` scheitert. Kein Multi-File, keine Governance-/Policy-Ziele, kein Builder-Core, keine Secrets-/Deploy-/Provider-Core-Ziele und keine freie class_2/class_3-Ausweitung ohne neuen separaten Entscheid.`
