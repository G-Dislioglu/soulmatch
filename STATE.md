...
- `next_recommended_block`: `Nach dem live verifizierten Praezisions-Fix bf36900 ist der naechste sinnvolle enge Schnitt kein weiterer Scope-Ausbau, sondern eine Design-/Produktentscheidungsrunde: ob `BACKTICK_IN_REGEX` bei besserer Heuristik weiter `block` bleiben soll, welche Checks ausser `checkLengthLimits` bewusst global bleiben, und ob spaeter echte Verlaufstelemetrie statt nur `latestObservation` benoetigt wird. Cross-Run-Persistenz oder AICOS-Card-Truth-Sync sollten erst dann folgen, wenn ihre lokale Repo-Grundlage konkret belegt ist.`
