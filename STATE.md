...
- `last_completed_block`: `S35-F12 im lokalen Working Tree: Architecture-Digest als vierter Context-Broker-Endpoint gebaut. Neuer Helper server/src/lib/architectureDigest.ts liefert deterministisch Module-, Routes-, DB-Table-, Cross-Repo- und Conventions-Struktur mit 5-Min-In-Memory-Cache und optionalem sections-Filter. Kritische Zuschnitte gegen die Spec: Modul-purpose via Kommentar-Extraktion mit Fallback statt MODULE.md-Pflicht; Modul-dependencies werden aus allen .ts/.tsx-Dateien pro Modul abgeleitet, nicht nur aus index.ts; Routes spiegeln die realen app.use-Mounts aus server/src/index.ts statt Beispielpfade; fuer Client-/Root-Quellen gilt lokal-first mit GitHub-fallback, weil der Runtime-Container keinen kompletten Repo-Checkout hat. Neuer Endpoint POST /api/context/architecture-digest in contextBroker.ts. Server-TSC lief lokal gruen.`
- `next_recommended_block`: `F12 pushen und live pruefen: (1) voller Digest ohne Body, (2) sections-Filter wie {sections:["routes"]}, (3) Cache-Hit innerhalb 5 Minuten ueber identisches generatedAt, (4) danach entscheiden, ob Claude-Session-Start standardmaessig session-start plus architecture-digest hintereinander ziehen soll. Sekundaer bleiben alte Backlog-Themen wie Maya-Core-Cut, Kaya-Rename und Graph-/Fusion-Spec.`
