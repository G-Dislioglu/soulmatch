...
- `last_completed_block`: `Phase 1 der Internal-Architect-Control-Plane ist repo-sichtbar und live verifiziert: Commit 47deb25 baut in server/src/lib/architectPhase1.ts einen AICOS-Meta-Loader, eine Assumption-Registry, das finale Assembly-/Dispatch-Gate und den read-only Endpoint /api/architect/state; opusTaskOrchestrator.ts sowie die sync/async Opus-Task-Pfade tragen jetzt metaSourceIds, assumptions und assumptionIds. Die neue Tabelle builder_assumptions wurde produktiv ueber den geschuetzten Remote-Pfad /api/builder/opus-bridge/migrate ausgerollt. Build-Pruefung lief gruen ueber cd server && pnpm build und cd client && pnpm typecheck. Live-Verifikation: Render wechselte erst auf 47deb25 und danach fuer den Hotfix auf 4a072ec; /api/architect/state zeigt persistenceMode=database; der produktive Gate-Smoke blockt eine vergiftete Assumption vor dem Worker-Swarm. Der Hotfix 4a072ec korrigiert dabei einen echten False-Positive im secret_exfiltration_pattern, das harmlose AICOS-Felder wie Token: zuvor als blocked markierte. Der enge Hardening-Followup 910fbed haertet `server/src/lib/specHardening.ts` weiter: die zentralen Forbidden-Patterns sind jetzt code-block-aware und matchen nur noch in fenced Markdown-Code statt auch in reiner Prosa. Lokal liefen server build + client typecheck gruen, ein systematischer Vier-Pattern-Smoke (`<?php`, `<script`, `DROP TABLE`, `rm -rf /`) blieb in Prosa frei und blockte weiter in Code-Fences, und live wurde ein Async-Dry-Run mit prose-eingebetteten Triggerwoertern akzeptiert statt vom Hardening-Gate geblockt. Der anschliessende Observability-Block 0e4c075 erweitert `server/src/lib/architectPhase1.ts` um kompakte Aggregationen fuer Findings, Truncation und letzte Block-/Warn-Signale in `latestObservation`; der Counter heisst bewusst `observedAssemblies`, weil nicht alle fruehen Return-Pfade bereits in `updateObservation()` laufen. Lokal liefen server build + client typecheck erneut gruen, und live zeigte `/api/architect/state` vor dem Smoke `observedAssemblies=0` und nach einem harmlosen Async-Dry-Run `observedAssemblies=1` bei weiter leeren Block-/Warn- und Truncation-Counts. Der naechste enge Hardening-Block a5220e1 macht nun auch `checkBacktickRegexPattern`, `checkUnescapedBackslashes`, `checkCurlyTemplatePattern` und `checkQuoteImbalance` code-aware, ohne Regexe, Severities oder Finding-Codes zu aendern; `checkLengthLimits` blieb global. Der lokale Vierer-Smoke zeigte dieselben Trigger in Prosa frei und in fenced Code weiter aktiv, und live lief ein erlaeuternder Async-Dry-Run mit `/foo/`, Backticks, Escape-Sequenzen, Template-Placeholders und einem Quote-Fragment in Prosa durch; `observedAssemblies` stieg dabei von 1 auf 2, Block-/Warn-Signale blieben leer. Der nachgezogene Praezisions-Fix bf36900 verengt `BACKTICK_IN_REGEX` weiter auf echte backtick-delimited Regex-Spans statt auf jede Zeile mit Slash plus zwei Backticks. Lokal verschwand damit der bekannte False-Positive-Fall `console.log("/api/users"); /* doc \`text\` and more \`text\` */`, waehrend `const re = \`/foo/\`;` weiter blockte und Prosa frei blieb. Live lief ein harmloser Async-Dry-Run mit genau diesem False-Positive-Codebeispiel durch; `observedAssemblies` stieg von 0 auf 1, Block-/Warn-Signale blieben leer.`
- `next_recommended_block`: `Die enge Design-/Produktentscheidungsrunde zum Hardening ist vorerst geschlossen: `BACKTICK_IN_REGEX` bleibt nach dem Praezisions-Fix bf36900 auf `block`, global bleibt nur `checkLengthLimits`, und zusaetzliche Verlaufstelemetrie wird bewusst erst bei echtem Entscheidungsdruck gebaut. Der naechste sinnvolle Produktschnitt ist deshalb kein weiterer Hardening-Ausbau, sondern nur dann neue Arbeit, wenn konkrete False-Positives, echte Severity-Zweifel oder belastbare lokale AICOS-Artefakte fuer einen Folgeblock auftauchen; die offene Render-Deploy-Abweichung des docs-only Pushes bleibt ein separater Betriebsfall.`
