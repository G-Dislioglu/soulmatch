...
- `next_recommended_block`: `Nach dem live verifizierten Observability-Block 0e4c075 ist der naechste sinnvolle enge Schnitt kein weiterer State-Ausbau, sondern Phase 2B: Guard-False-Positive-Raender in `server/src/lib/specHardening.ts` gezielt testen und schaerfen, insbesondere Backtick-/Quote-/Length-Heuristiken, die noch nicht denselben Kontextstandard wie die code-block-aware Forbidden-Patterns haben. Cross-Run-Persistenz, Capability-Register oder meta-008-artige Erweiterungen sollten erst danach folgen.`
