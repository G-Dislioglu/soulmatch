...
- `next_recommended_step`: Keinen breiten Capability- oder meta-008-Ausbau auf denselben Pfad legen. Wenn weitergemacht wird, dann erst bei neuem repo-belegtem Bedarf: konkreter False-Positive-Schmerz, reale Severity-Zweifel oder ein belastbarer lokaler AICOS-Folgeblock. Der aktuelle Lokalbestand reicht dafuer noch nicht: repo-sichtbar existieren als eigene `sol-cross`-Dateien nur `013`, `031`, `034`, `055` und `056`; `057` bis `061` fehlen lokal, und `sol-cross-060` taucht bisher nur als Textreferenz in `aicos-registry/treegraphos/hardening/FIELD-HARDENING-RUN-003.md` auf. Ohne Klaerung dieser Herkunft ist auf diesem Slice weiter bewusst Nicht-Bauen die richtige Entscheidung.
