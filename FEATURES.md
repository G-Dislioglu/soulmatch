...
- `quality`: `phase1_architect_gate_live_with_observability_metrics`
- `known_gap`: Die fruehere Phase-0-Kontrollschicht ist jetzt in einen echten Phase-1-Pfad uebergegangen: `architectPhase1.ts` fuehrt AICOS-Meta-Loader, Assumption-Registry, finale Assembly und Dispatch-Gate ein; `/api/architect/state` zeigt den Beobachtungszustand read-only; `builder_assumptions` ist produktiv aktiv. `specHardening.ts` ist seit 910fbed fuer die wichtigsten Forbidden-Patterns code-block-aware, und 0e4c075 macht Findings-, Truncation- und Signal-Counts im State sichtbar. Offen bleibt damit nicht die Grundfunktion des Pfads, sondern die Restpraezision der uebrigen Guard-Heuristiken ausserhalb des gehaerteten Prosa-vs-Code-Schnitts und die noch ehrliche Begrenzung des Counters auf `observedAssemblies` statt vollstaendige Totalzaehlung.
- `next_recommended_step`: Keinen breiten Capability- oder meta-008-Ausbau auf denselben Pfad legen; wenn weitergemacht wird, dann zuerst als enger Guard-False-Positive-Block fuer Backtick-/Quote-/Length-Heuristiken in `specHardening.ts`, nun auf Basis der live sichtbaren State-Metriken.
- `evidence`: `server/src/lib/builderControlPlane.ts`, `server/src/lib/directorContext.ts`, `server/src/lib/specHardening.ts`, `server/src/lib/architectPhase1.ts`, `server/src/lib/opusTaskOrchestrator.ts`, `server/src/routes/architect.ts`, `server/src/routes/health.ts`, `server/src/routes/opusBridge.ts`, `server/src/schema/builder.ts`, `server/migrations/0001_builder_assumptions.sql`; Feature-Commit `47deb25` live verifiziert am 2026-04-24, Hotfix `4a072ec` ebenfalls live verifiziert, Guard-Hardening `910fbed` live verifiziert am 2026-04-25, Observability-Block `0e4c075` live verifiziert am 2026-04-25, produktive Migration ueber `/api/builder/opus-bridge/migrate`, `/api/architect/state` mit `persistenceMode=database`, Live-Gate-Smoke `job-mod99ozd` blockt die Poison-Assumption vor dem Worker-Swarm bei akzeptierten Meta-Karten `meta-001` bis `meta-007`, ein weiterer Live-Dry-Run mit prose-eingebetteten Triggerwoertern wurde vom Async-Path akzeptiert statt vom Hardening-Gate geblockt, und der Observability-Smoke hob `observedAssemblies` live von 0 auf 1 bei weiterhin leeren Block-/Warn- und Truncation-Counts.`
