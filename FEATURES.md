...
- `known_gap`: Builder blockt jetzt bekannte Blacklist-Dateien schon im Chat, erklaert geblockte Laeufe genauer, stoppt stale Tasks serverseitig nach Timeout automatisch und zeigt den Grund ueber Maya-/Opus-Logs in der bestehenden Observe-Ansicht. Quick-Mode-Tasks laufen ueber `executeTask`, `agentHabitat` speichert pro Worker drei kompakte Nachdenker-Learnings fuer den naechsten Prompt, offene Tasks koennen direkt in der Builder-UI manuell geblockt werden, und Maya hat jetzt zusaetzlich einen Director-Modus mit internen Action-Blocks. Der Live-Nachweis fuer bewusste Director-Aktionen ist auf Render jetzt fuer `read-file` erbracht; offen bleiben der echte delegierte Async-Opus-Task, belastbarer Async-Jobstatus und die UI-Sichtbarkeit der ausgefuehrten Schritte. Die persistente Ebene braucht auf Render aber weiter den manuellen Schema-Push fuer `builder_memory` und `builder_agent_profiles.last_learnings`; bis dahin bleibt die neue Memory im Deploy nur RAM-gestuetzt plus soft-failing DB-Zugriffe.
- `next_recommended_step`: Auf Render einen echten Director-Flow mit delegiertem Async-Opus-Task pruefen, dabei Jobstatus und Builder-Studio-Sichtbarkeit verfolgen und parallel `[stale-detector]`, `blocked`, ChatPool-Hinweis und `builder_opus_log` gegen den Live-Lauf abgleichen.
