...
- `known_gap`: Der eigentliche Repo-Deploypfad ist bestaetigt, und `tools/wait-for-deploy.sh` behandelt seit `890b03b` auch den docs-only bzw. non-runtime `HEAD`-Push sauber: Solche Commits skippen frueh statt einen nicht stattfindenden Render-Rollout zu erwarten. Offen bleibt nicht mehr der normale `HEAD`-Fall, sondern nur noch die konservative Pfadklassifikation und das bestehende lokale Operator-Gap: Auf dem Windows/bash-Client ist die DNS-Aufloesung fuer `soulmatch-1.onrender.com` weiter unzuverlaessig, waehrend dieselbe Runtime ueber `--resolve` bzw. `DEPLOY_RESOLVE_IP=216.24.57.7` sauber erreichbar bleibt.
- `next_recommended_step`: Keinen weiteren Deploy-Umbau vorziehen. Nur dann nochmals an `tools/wait-for-deploy.sh` arbeiten, wenn ein echter Nicht-HEAD-Wait-Fall, eine falsch klassifizierte non-runtime Datei oder ein neuer CI-Drift repo-sichtbar auftaucht; den lokalen DNS-Resolve-Drift separat beobachten.
- `evidence`: `.github/workflows/render-deploy.yml`, `tools/wait-for-deploy.sh`, `README.md`, `DEPLOY.md`, `server/src/routes/health.ts`; Live-Probe 2026-04-24 nach Push `d381d00 -> a53dd69`: Standard-curl lokal mit Host-Resolve-Failure, aber `DEPLOY_RESOLVE_IP=216.24.57.7 EXPECT_COMMIT=$(git rev-parse HEAD) bash tools/wait-for-deploy.sh` wechselte nach 165s auf `a53dd69`, und direkter `curl --resolve soulmatch-1.onrender.com:443:216.24.57.7 https://soulmatch-1.onrender.com/api/health` lieferte HTTP 200 plus `commit: a53dd69...`. Followup 2026-04-25: `890b03b` fuehrt den non-runtime-HEAD-Skip fuer `*.md`, `docs/**`, `tools/wait-for-deploy.sh` und `.github/workflows/render-deploy.yml` ein; lokal gruen verifiziert ueber `bash -n`, docs-only-`HEAD`-Skip, Gegencheck gegen `bf36900` als wait-pflichtigen Code-Commit und die Standard-Invocation `DEPLOY_RESOLVE_IP=216.24.57.7 EXPECT_COMMIT=$(git rev-parse HEAD) bash tools/wait-for-deploy.sh` nach dem Push.`
...
- `known_gap`: Die Basis fuer F14A ist live verifiziert; lokal wurde jetzt zusaetzlich ein enger v1.2-Operator-Gate-Vertrag umgesetzt (tri-state `decision`, `requiredExternalApproval`, `approvalId`, class_2 Plan/Approval-Gate, uncertain=>push-block). Diese v1.2-Erweiterung ist noch worktree-only und braucht gezielte Payload-Verifikation plus Commit/Push, bevor sie als remote Wahrheit gilt.
- `next_recommended_step`: Drei gezielte `/opus-task`-Checks fahren und erst danach committen: (1) class_2 ohne `approvalId`/`hasApprovedPlan` muss `dry_run_only` liefern, (2) class_2 mit `approvalId`+`hasApprovedPlan` darf bei `decision=approve` push-faehig sein, (3) `decision=uncertain` muss Push blocken und `requiredExternalApproval=true` ausgeben.
- `evidence`: `server/src/lib/providers.ts`, `server/src/lib/poolState.ts`, `server/src/lib/opusScoutRunner.ts`, `server/src/lib/opusRoundtable.ts`, `server/src/lib/opusBridgeController.ts`, `server/src/lib/opusDistiller.ts`, `server/src/lib/opusJudge.ts`, `server/src/lib/opusClaimGate.ts`, `server/src/lib/opusTaskOrchestrator.ts`, `server/src/lib/opusWorkerSwarm.ts`, `server/src/lib/opusWorkerRegistry.ts`, `server/src/routes/opusBridge.ts`, `server/src/lib/builderSafetyPolicy.ts`, `docs/F14-PHASE1-CLAIM-GATE-SPEC.md`; Live-Probe `ad8abd0a2ac6e2cf6419f8598c0d524efbe7d127` erzeugte `docs/S31-MULTIFILE-PROBE-a.md`, `docs/S31-MULTIFILE-PROBE-b.md` und `docs/S31-MULTIFILE-PROBE-c.md` in genau einem Remote-Commit. Zusaetzlich ist F14A auf Production ueber `/api/health/opus-task-async` gegen `d375304` runtime-verifiziert: `job-mobq28w2` bestaetigt `anchored + compatible` plus Judge-Approve, `job-mobq72r1` bestaetigt `anchored + mismatch` plus Judge-Reject, `job-mobpxh4b` bestaetigt den Fresh-Check fuer realen unindexierten manual scope, `job-mobpt6rd` bestaetigt den Fruehreject fuer einen nicht existierenden Pfad ohne Create-Signal; v1.2 selbst aktuell lokal verifiziert via erfolgreichem `cd server && pnpm build`.
