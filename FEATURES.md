...
- `known_gap`: Builder blockt jetzt bekannte Blacklist-Dateien schon im Chat und erklaert geblockte Laeufe genauer; zusaetzlich ist eine separate Maya-Dashboard-Flaeche ueber `/maya` sichtbar verdrahtet. Quick-Mode-Tasks laufen jetzt ueber `executeTask`, und `agentHabitat` speichert pro Worker drei kompakte Nachdenker-Learnings fuer den naechsten Prompt. Die persistente Ebene braucht auf Render aber weiter den manuellen Schema-Push fuer `builder_memory` und jetzt auch `builder_agent_profiles.last_learnings`; bis dahin bleibt die neue Memory im Deploy nur RAM-gestuetzt plus soft-failing DB-Zugriffe.
- `next_recommended_step`: Nach einem erfolgreichen Render-`drizzle-kit push` einen echten Builder-Chat- oder Task-Lauf gegen die Zielruntime fahren und pruefen, ob Episode-, Semantic-, Worker-Profil- und `last_learnings`-Eintraege wirklich geschrieben werden.
- `evidence`: `client/src/app/App.tsx`, `client/src/modules/M16_builder/ui/MayaDashboard.tsx`, `client/src/modules/M16_builder/hooks/useMayaApi.ts`, `server/src/lib/builderGithubBridge.ts`, `.github/workflows/builder-executor.yml`, `server/src/lib/builderMemory.ts`, `server/src/lib/builderFusionChat.ts`, `server/src/lib/builderDialogEngine.ts`, `server/src/lib/agentHabitat.ts`, `server/src/lib/opusBridgeController.ts`, `server/src/routes/builder.ts`, `server/src/schema/builder.ts`.
