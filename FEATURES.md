...
- `known_gap`: Builder blockt jetzt bekannte Blacklist-Dateien schon im Chat, erklaert geblockte Laeufe genauer, stoppt stale Tasks serverseitig nach Timeout automatisch und zeigt den Grund ueber Maya-/Opus-Logs in der bestehenden Observe-Ansicht. Quick-Mode-Tasks laufen ueber `executeTask`, `agentHabitat` speichert pro Worker drei kompakte Nachdenker-Learnings fuer den naechsten Prompt, offene Tasks koennen direkt in der Builder-UI manuell geblockt werden, und Maya hat jetzt zusaetzlich ein `Maya Brain` mit internen Action-Blocks, Modell-Dropdown (`Opus 4.6`, `GPT 5.4`, `GLM 5.1`), `Fast`/`Deep`-Thinking, sichtbaren Action-Badges, explizitem Fast/Deep-Modus im Director-Prompt, lokalem Lauf-/Toolstatus in `BuilderStudioPage` und automatischer Continuity-Rueckschreibung pro Brain-Interaktion. Diese Steuerelemente sitzen jetzt direkt in der aktiven `/builder`-Oberflaeche `BuilderStudioPage`. Ueber `memory-read` und `memory-write` greift dieselbe Maya auch direkt auf `builder_memory` zu. GLM 5.1 ist repo-sichtbar auch als Meister und Worker verdrahtet. Scout, Destillierer und Worker koennen jetzt ueber denselben Observe-Pfad als Pool-Chat im Builder gelesen werden; Builder- und Opus-Token werden dafuer getrennt im Browser gehalten, und die frueheren Bottom-Widgets sind durch PoolBar-Popups ersetzt. Offen bleiben der echte delegierte Async-Opus-Task auf Render, belastbarer Async-Jobstatus, Council als eigener Chat-Block ohne Moderationsdrift und die Live-Pruefung, ob Fast unter echten Provider-Keys den vorgesehenen Toolpfad stabil nutzt. Die persistente Ebene braucht auf Render aber weiter den manuellen Schema-Push fuer `builder_memory` und `builder_agent_profiles.last_learnings`; bis dahin bleibt die neue Memory im Deploy nur RAM-gestuetzt plus soft-failing DB-Zugriffe.
- `next_recommended_step`: Council nur als eigenen naechsten Pool-Block schneiden, wenn die vorhandene PoolBar-Konfiguration und Maya-Moderation nicht verwischt werden; parallel den Maya-Brain-Flow mit delegiertem Async-Opus-Task auf Render pruefen.
