...
- `known_gap`: Scout, Distiller, Roundtable und Worker nutzen fuer GLM jetzt OpenRouter statt Direkt-Zhipu, und der Opus-Task-Pfad traegt neue Dateien explizit als `create` durch Scope, Prompt und Push. Der End-to-End-Nachweis auf Render fuer Datei-Injektion, echte BDL-Ausgabe auf eigener Zeile, automatischen GitHub-Commit, Distiller-Treue bei `getWorstPerformers`, semantische Judge-Strenge gegen die Original-Instruktion und stabile GLM-Laufzeit unter OpenRouter steht aber noch aus.
- `next_recommended_step`: Einen echten Builder-/Opus-Task gegen die Zielruntime fahren und dabei pruefen, ob `getWorstPerformers` im Titel und Brief erhalten bleibt, ob `@READ` als `[system]`-Dateiinhalt ankommt, ob neue Dateien als `create` statt als implizites Overwrite geloggt werden und ob `GITHUB_PAT` den Dispatch/Commit wirklich erlaubt.
